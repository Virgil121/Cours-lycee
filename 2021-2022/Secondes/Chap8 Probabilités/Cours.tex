...
\begin{scope}
\clip (1,0) circle (1.5);
\draw[fill=black,pattern=north east lines,draw=none,opacity=0.5] (-1,0) circle (1.5);
\end{scope}

%\node (inter) at (0,-1.7) {$A \cap B$};
%\draw (inter) -- (0,-0.5);
...
\draw[fill=black,pattern=north east lines,draw=none,opacity=0.5] (-1,0) circle (1.5) (1,0) circle (1.5);
...
\draw[fill=black,pattern=north east lines,draw=none,opacity=0.5,even odd rule] (-2,-2) rectangle (2,2) (0,0) circle (1.5);
...
\begin{prop}
Dans ce cas, chaque issue a pour probabilité $\frac 1 {\textrm{nombre total d'isues}}$.
