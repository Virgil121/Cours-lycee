...
Lorsque deux vecteurs symbolisent le même déplacement, on dit qu'ils sont égaux. La figure précédente nous donne alors $\vecc{AB} = \vecc {MN}, \vecc{BA} = \vecc{NM}, \vecc {AM} = \vecc {BN}$.
...
\textbf{Méthode :} Pour construire $\vecc {AB} + \vecc {AC}$, il suffit de construire le parallélogramme $ABDC$ et de prendre le vecteur $\vecc {AD}$.
