...
\newcommand{\compofix}{\vspace{0.7\baselineskip}}

\begin{document}

\rem[black]{L'usage de la calculatrice est autorisé. La propreté et l'orthographe seront prises en compte. Tout le devoir peut être fait sur cette feuille.}
...
$A=\left( \frac {-5} 3 \right)^1 \times \left( \frac {-5} 3 \right)^2$ \hfill $B=\frac {\left(2^3\right)^8 \times 2^{-10}} {2^{14}} $ \hfill $C=\frac { 12^5 \times 4^2 } {3^2 \times 4^3}$
\end{minipage}
\end{center}
\vspace{70mm}
\end{exercice}
...
\begin{enumerate}
\item $\vecc {AH} + \vecc {HL} = \makebox[3cm]{\dotfill}$
\item $\vecc {CD} - \vecc {CF} + \vecc {DF} = \dotfill$
\end{enumerate}
\end{exercice}
...
\draw[line width=1pt,->,>=latex] (-14,6) -- ++(4,-2) node[pos=0.5,above] {$\vec u$};
\draw[line width=1pt,->,>=latex] (-7,4) -- ++(-3,3) node[pos=0.5,above right] {$\vec v$};
\draw[line width=1pt,<-,>=latex] (-11,3) -- ++(-3,-9) node[pos=0.5,above left] {$\vec w$};
...
\begin{enumerate}
\item Placer le point M tel que $\vecc{AM}=\vec u+2 \vec v$
\item Placer le point N tel que $\vecc{BN} = \vecc{AB} - \vec w$
\item Placer le point O tel que $\vecc{CO} = \vec v+\frac 1 2 \vec u+\frac 2 3 \vec w$
\item Placer le point I, milieu de $[AC]$.
\item Compléter: $\vecc {AI} = \makebox[1cm]{\dotfill} \vecc{AC}$
\end{enumerate}
\end{exercice}
...
\compo[0.65]
{
\quad On considère le triangle $ABC$, et les points $D \in [AB], E \in [AC]$ tels que $(DE) \parallel (BC)$, comme sur le schéma ci-contre.
\begin{enumerate}
\item Compléter: $\vecc {AD} = \makebox[15mm]{\dotfill} \vecc {AB}$, $\vecc {AE} = \makebox[3cm]{\dotfill} $
\item Compléter puis justifier: $\vecc {DE} = \makebox[3cm]{\dotfill} $
\end{enumerate}
\points 3
}
{
...
\begin{enumerate}
\item Tracer un vecteur $\vec v$ ayant la même longueur que $\vec u$, mais différent de $\vec u$.
\item Que dire du vecteur $\vec w$ par rapport au vecteur $\vec u$ ?
\end{enumerate}
\points 2
}
{
...
\draw[line width=1pt,->,>=latex] (1,1) -- (4,7) node[pos=0.5,left] {$\vec u$};
\draw[line width=1pt,->,>=latex] (14,6) -- (12,2) node[pos=0.5,right] {$\vec w$};
\end{tikzpicture}
\end{center}
}
\end{exercice}
...
En utilisant le repère ci-contre:
\begin{enumerate}
\item Comparer trois caractéristiques des vecteurs $\vec u$ et $\vecc {BD}$. \points 3
\item Tracer deux représentants du vecteur $\vecc {AB}$, l'un d'origine $D$, l'autre d'extrémité $C$.
\item Placer $M$, l'image de $C$ par la translation de vecteur $\vecc {DB}$.
\item Placer $N$, l'image de $C$ par la translation de vecteur $\vecc {DC}$.
