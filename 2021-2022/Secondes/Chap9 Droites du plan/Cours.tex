...
\begin{methode}
Pour tracer une droite $\mathscr D$ étant donnée son équation cartésienne, on détermine les coordonnées de deux points appartenant à $\mathscr D$ en remplaçant $x$ ou $y$ par des valeurs spécifiques.
\end{methode}
...
\begin{itemize}
\item Si $x=0$, alors l'équation devient $\begin{aligned}[t] -y+3 & =0 \\ y & =3 \end{aligned}$

Alors $A(0;3) \in \mathscr D$.
\item Si $x=1$, alors l'équation devient $\begin{aligned}[t] 2-y+3 & =0 \\ y & =5 \end{aligned}$

Alors $B(1;5) \in \mathscr D$.
\end{itemize}
...
\node[stylepoint,fill=blue,label=-45:$A$] at (0,3) {};
\node[stylepoint,fill=blue,label=-45:$B$] at (1,5) {};

\draw[line width=1pt,DarkRed] (3,-2) -- (3,11) node[pos=0.65,right] {$\mathscr D_2$};
\node[stylepoint,fill=red,label=0:$C$] at (3,1) {};
\node[stylepoint,fill=red,label=0:$D$] at (3,4) {};
\end{axis}
\end{tikzpicture}
\end{center}
}
Soit $\mathscr D_2:3x-9=0$. Ici, il n'y a pas de $y$ donc il suffit de résoudre l'équation: $\begin{aligned}[t] 3x-9 & =0 \\ 3x & =9 \\ x & =3 \end{aligned}$

Tout point dont les coordonnées sont de la forme $(3;y)$ avec $y \in \R$ convient donc. On prend par exemple $C(3;1)$ et $D(3;4)$.
\end{exs}
