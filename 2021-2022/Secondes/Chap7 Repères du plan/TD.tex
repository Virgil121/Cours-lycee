...
\draw[stylevecteur] (-4,3) -- ++(5,-1) node[pos=0.5,above] {$\vv u$};
\draw[stylevecteur] (4,4) -- ++(-2,-3) node[pos=0.5,above left] {$\vv v$};
\draw[stylevecteur] (-3,-1) -- ++(11,2) node[pos=0.5,above] {$\vv w$};
\draw[stylevecteur] (8,0) -- ++(-3,3) node[pos=0.5,above right] {$\vv \alpha$};
\end{tikzpicture}
\end{resizetikz}
\end{centrer}
\end{exercice}
...
\item En déduire les coordonnées des vecteurs $\vv{AB}$, $\vv{AC}$ et $\vv{BC}$.
...
\begin{exercice}
Soient $A(-2;3)$ et $\vv u \binom 4 {-2}$. Calculer les coordonnées du point $B$ image de $A$ par la translation de vecteur $\vv u$.
\end{exercice}
...
\begin{exercice}[*]
Soient $A(7;-1)$, $B(-4;10)$ et $C(-4;5)$. Déterminer les coordonnées du point $M$ tel que $\vv{MA} + \vv{MB} + \vv{MC}=\vv 0$.
\end{exercice}
...
\begin{exercice}[*]
On se donne les points $A(-2;-3)$, $B(5;0)$, $C(0;7)$.
\begin{enumerate}
\item Calculer les coordonnées de $I$, $J$ et $K$, milieux respectifs de $[BC]$, $[AC]$ et $[AB]$.
\item Déterminer les coordonnées des points $G$, $H$ et $L$ tels que $\vv{AG} = \frac 2 3 \vv{AI}$, $\vv{BH} = \frac 2 3 \vv{BJ}$ et $\vv{CL} = \frac 2 3 \vv{CK}$.
...
\begin{enumerate}
    ...
    \begin{centrer}
    	\begin{resizetikz}{0.8\linewidth}
        \begin{tikzpicture}
        \repereOij[xmin=-6,xmax=6,ymin=-5,ymax=5]
        \end{tikzpicture}
        \end{resizetikz}
    \end{centrer}
...
\centering{\color{gray}\rule{0.8\linewidth}{0.4pt}}

%\section{Exercices supplémentaires}

\begin{exercice}
Soient $A(-3;1)$ et $B(1;4)$. Déterminions le point $M$ de l'axe des abscisses qui se trouve à égale distance de $A$ et $B$. Notons alors $(x;0)$ les coordonnées de $M$.
\begin{enumerate}
\item Exprimer les distances $MA$ et $MB$ en fonction de $x$.
\item Résoudre l'équation $MA^2=MB^2$ et conclure.
%\item Conclure l'étude.
\end{enumerate}
\end{exercice}
...
Considérons une homothétie de centre $A$ et de rapport $k$.
\begin{enumerate}
...
\begin{centrer}
    \begin{resizetikz}{0.8\linewidth}
    \begin{tikzpicture}
    \repereOij[xmin=-5,xmax=9,ymin=-3,ymax=5]
    \draw[fill=white] (-1,1) -- ++(1,0) -- ++(0,1) -- ++ (2,0) -- ++ (0,1) -- ++ (-3,0) -- cycle;
    \draw (0,2.5) node{$f$};
    \draw (-4,4) node{$\times$};
...
\begin{exercice}[*]
Dans un repère orthonormé, on donne les points $A(11;-3)$, $B(8;-3+3 \sqrt 3)$, $C(2;-3+3 \sqrt 3)$.
\begin{enumerate}
\item Placer ces points.
\item Démontrer que le triangle $ABC$ est isocèle en $B$.
\item Déterminer les coordonnées du point $I$ tel que $ABCI$ soit un parallélogramme.
\item Démontrer que les points $A$, $B$ et $C$ appartiennent à un même cercle $\mathcal C$ de centre $I$.
\item Tracer ce cercle $\mathcal C$ et construire les points $A'$, $B'$ et $C'$ diamétralement opposés respectivement à $A$, $B$ et $C$.
\item Quelle est la nature du polygone $ABCA'B'C'$?
\end{enumerate}
\end{exercice}
...
\draw[->,>=latex,line width=1pt,cap=round,color=black!30!red] (0,0) -- (1,0) node[pos=0.5,below] {$\vv i$};
\draw[->,>=latex,line width=1pt,cap=round,color=black!30!red] (0,0) -- (0,1) node[pos=0.5,left] {$\vv j$};
% \vv
\draw[stylevecteur] (-4,3) -- ++(5,-1) node[pos=0.5,above] {$\vv u$};
\draw[stylevecteur] (4,4) -- ++(-2,-3) node[pos=0.5,above left] {$\vv v$};
\draw[stylevecteur] (-3,-1) -- ++(11,2) node[pos=0.5,above] {$\vv w$};
...
% \vv
\draw[stylevecteur] (-4,3) -- ++(axis direction cs:5,-1) node[pos=0.5,above] {$\vv u$};
\draw[stylevecteur] (4,4) -- ++(axis direction cs:-2,-3) node[pos=0.5,above left] {$\vv v$};
\draw[stylevecteur] (-3,-1) -- ++(axis direction cs:11,2) node[pos=0.5,above] {$\vv w$};
\draw[stylevecteur] (8,0) -- ++(axis direction cs:-3,3) node[pos=0.5,above right] {$\vv \alpha$};
\end{axis}
\end{tikzpicture}
\begin{resizetikz}{0.9\linewidth} % Repère quelconque à la main
\begin{tikzpicture}
