...
\item Expression des coordonnées de $\vv{AB}$ en fonction de celles de $A$ et $B$.
...
\begin{defin}
Soient $O,I,J$ trois points du plan non alignés. On pose $\vv i = \vv{OI}$ et $\vv j = \vv{OJ}$. Un repère du plan est un triplet \Oij. On dit alors que :
\begin{itemize}
...
\draw[stylevecteur,color=black!30!red] (0,0) -- (1,0) node[pos=0.5,below] {$\scriptstyle \vv i$};
\draw[stylevecteur,color=black!30!red] (0,0) -- (0,1) node[pos=0.5,left] {$\scriptstyle \vv j$};
...
\item Les vecteurs unité $\vv i$ et $\vv j$ ont pour longueur 1:  $\Vert \vv i \Vert = \Vert \vv j \Vert=1$
\end{itemize}
}
{
\vspace{-2em}
...
\compo[0.75]
{
Soit $\vv u$ un vecteur du plan. On se donne le point $M(x;y)$ tel que $\vv{OM} = \vv u$. Les coordonnées de $\vv u$ sont celles de $M$, et l'on note $\vv u \dbinom x y$.
}
{\vspace{-2em}
\begin{center}
\begin{resizetikz}{0.9\linewidth}
...
\rem[eleve]{On écrit $\vv u \binom 3 2$ et pas $\vv u = \binom 3 2$ !}

\begin{ex}
\compo[0.6]
{
On a représenté ci-contre deux vecteurs $\vv u$ et $\vv v$.

On a $\vv u \dbinom 5 {-2}$, et $\vv v \dbinom {-2} {0}$.
}
{\vspace{-1.5em}
\begin{center}
\begin{resizetikz}{0.9\linewidth}
...
\draw[stylevecteur] (-3,3) -- (2,1) node[pos=0.5,above] {$\vv u$};
\draw[stylevecteur] (-2,1) -- (-4,1) node[pos=0.5,above] {$\vv v$};
\end{tikzpicture}
\end{resizetikz}
\end{center}
}
...
\begin{prop}
Soient $A(x_A;y_A)$ et $B(x_B;y_B)$ deux points du plan. Alors on a $\vv{AB} \dbinom {x_B-x_A} {y_B-y_A}$.
\end{prop}
...
Alors on a $\vv{AB} \dbinom {5-1}{-1-2}$ d'où $\vv{AB} \dbinom {4} {-3}$.

De même, on a $\vv{BC} \dbinom {6-5}{3-(-1)}$ d'où $\vv{BC} \dbinom 1 4$.
}
{\vspace{-1.5em}
\begin{center}
\begin{resizetikz}{0.9\linewidth}
...
\begin{proprs}
Soient $\vv u \dbinom {x_{\vv u}} {y_{\vv u}}$ et $\vv v \dbinom {x_{\vv v}} {y_{\vv v}}$ deux vecteurs du plan, et $\lambda \in \R$. Alors:
\begin{itemize}
\item Les coordonnées de $\vv u + \vv v$ sont $\dbinom {x_{\vv u}+x_{\vv v}} {y_{\vv u}+y_{\vv v}}$.
\item Les coordonnées de $\lambda \vv u$ sont $\dbinom {\lambda x_{\vv u}} {\lambda y_{\vv u}}$.
\end{itemize}
\end{proprs}
\begin{ex}
Soient $\vv u \dbinom 1 3$ et $\vv v \dbinom 2 {-1}$. Alors on a:
\begin{itemize}
\item $\left( \vv u + \vv v  \right) \dbinom {1+2} {3+(-1)}$ d'où $\left( \vv u + \vv v  \right) \dbinom 3 2$.
\item $3\vv v \dbinom {3 \times 2} {3 \times (-1)}$ d'où $3\vv v \dbinom {6} {-3}$.
\item $\left( 2 \vv u - 5\vv v \right) \dbinom {2 \times 1 -5 \times 2} {2 \times 3 -5 \times (-1)}$ d'où $\left( 2 \vv u - 5\vv v \right) \dbinom {-8}{11}$.
\end{itemize}
\end{ex}
...
\begin{prop}
Soit $\vv u \dbinom x y$ un vecteur du plan. Alors la norme de $\vv u$ est donnée par:
$$\Vert \vv u \Vert = \sqrt{x^2+y^2}$$
\end{prop}
...
\begin{defin}
Deux vecteurs $\vv u$ et $\vv v$ sont dits colinéaires s'il existe $\lambda \in \R$ tel que $\vv u = \lambda \vv v$. Dans le cas où ils sont non nuls, cela revient à dire qu'ils ont la même direction.
\end{defin}
...
\begin{exs}
On se donne $\vv u \dbinom 2 5$ et $\vv v \dbinom 4 {10}$. On remarque que $\vv v = 2 \vv u$. Alors $\vv u$ et $\vv v$ sont colinéaires.

\begin{center}
\begin{tabularx}{0.95\linewidth}{|c|c|Y|} \hline
	Coordonnées de $\vv u$ & Coordonnées de $\vv v$ & $\vv u$ et $\vv v$ sont-ils colinéaires? \\ \hline
	\rule[-16pt]{0pt}{38pt}$\dbinom 2 5$ & $\dbinom 4 {10}$ & $\vv v = 2 \vv u$ donc oui \\ \hline
	\rule[-16pt]{0pt}{38pt}$\dbinom 1 1$ & $\dbinom 7 7$ & $\vv v = 7 \vv u$ donc oui \\ \hline
	\rule[-16pt]{0pt}{38pt}$\dbinom 2 5$ & $\dbinom {-6} {-15}$ & $\vv v = -3 \vv u$ donc oui \\ \hline
	\rule[-16pt]{0pt}{38pt}$\dbinom 4 {11}$ & $\dbinom {-7} {-20}$ & \vspace{-5pt}$\left.\begin{matrix} 4 \times (-20) = -80 \\ -7 \times 11 = -77 \end{matrix}\right\}$ et $-80 \neq -77$ donc non \\ \hline
\end{tabularx}
\end{center}
...
\begin{defin}
Soient $\vv u \dbinom {x} {y}$ et $\vv v \dbinom {x'} {y'}$ deux vecteurs du plan. On appelle déterminant de $\vv u$ et $\vv v$ le réel: $$\det(\vv u;\vv v)=xy'-x'y$$
\end{defin}

\begin{ex}
Soient $\vv u \dbinom {4}{11}$ et $\vv v \dbinom {-7}{-20}$.
On a $\begin{aligned}[t] \det (\vv u ; \vv v) &= 4 \times (-20) - (-7) \times 11\\&= -80 +77 \\&=-3\end{aligned}$
\end{ex}
...
Soient $\vv a \dbinom {-15}{40}$ et $\vv b \dbinom {9}{-24}$.
On a $\begin{aligned}[t] \det (\vv a ; \vv b) &= -15 \times (-24) -9 \times 40\\&= 360 - 360\\&=0\end{aligned}$

Les vecteurs $\vv a$ et $\vv b$ sont donc colinéaires. (On a en fait $\vv u = -\frac 5 3 \vv v$).
\end{ex}
...
Alors on a $\vv{AB} \dbinom 9 6$ et $\vv{AC} \dbinom {-6}{-4}$.

On a $\det (\vv{AB};\vv{AC})=9 \times (-4) - (-6) \times 6 = -36+36=0$.
}
{
\begin{center}
\begin{resizetikz}{0.9\linewidth}
...
On en déduit que les vecteurs $\vv{AB}$ et $\vv{AC}$ sont colinéaires, d'où $(AB) \parallel (AC)$, et les points $A$, $B$ et $C$ sont alignés.
\end{ex}
...
%\draw[->,>=latex,thick] (0,0) -- (1,0) node[pos=0.5,below] {$\vv i$};
%\draw[->,>=latex,thick] (0,0) -- (0,1) node[pos=0.5,left] {$\vv j$};
