...
\begin{ex}
Soient $A \dbinom 1 7$ et $B \dbinom 6 {-5}$.
...
Donc $\vec u$ et $\vec v$ sont colinéaires. (On a en fait $\vec u = -\frac 5 3 \vec v$).
\end{ex}

\subsection{Applications en géométrie}

\begin{methode}
On peut montrer que des droites sont parallèles ou l'alignement de points grâce à la colinéarité.
\end{methode}
...
On en déduit que les vecteurs $\vec{AB}$ et $\vec{AC}$ sont colinéaires, d'où $(AB) \parallel (AC)$. Comme ces deux droites sont confondues, les points $A$, $B$ et $C$ sont alors alignés.
\end{ex}
