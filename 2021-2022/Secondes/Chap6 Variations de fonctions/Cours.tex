...
\begin{itemize}
\item On dit que $f$ est constante sur $I$ si elle prend toujours la même valeur: Pour $x,y \in I$, on a $f(x)=f(y)$.
...
{
%\vspace{-1.5em}
On se donne la fonction $f$, définie sur $\left[-2;5\right]$ et représentée ci-contre.
On "résume" la courbe représentative de $f$ sous forme du tableau de variations suivant:
\begin{center}
\begin{tikzpicture}
\tkzTabInit[lgt=1.4,espcl=1.7]{$x$/1, $f(x)$/2}{$-2$,$-1$,2,5}
...
\begin{rmq}
Le maximum d'une fonction correspond au point le plut "haut" de sa courbe représentative, et le minimum au point le plus "bas".
\end{rmq}
...
\addplot[styleplot,dashed,color=DarkGreen] plot {4} node[pos=0.8,above,color=DarkGreen!50!black] {\textbf{maximum}};
\addplot[styleplot,dashed,color=DarkGreen] plot {-1} node[pos=0.8,below,color=DarkGreen!50!black] {\textbf{minimum}};
\end{axis}
\end{tikzpicture}
\end{center}
}
