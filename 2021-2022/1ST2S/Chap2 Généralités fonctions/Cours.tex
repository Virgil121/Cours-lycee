...
\Centering{$f(x)>k$} & \Centering{$f(x)<k$} & \Centering{$f(x)>g(x)$} \\ \hline
\multicolumn{2}{|c|}{
\begin{tikzpicture}[scale=1]
\begin{axis}[
axis x line=bottom,
...
\begin{defin}
Le taux de variation entre $a$ et $b$ d'une fonction $f$ définie sur un intervalle $I$ est le quotient $\frac{f(b)-f(a)} {b-a}$ pour $a$ et $b$ distincts et appartenant à $I$.
\end{defin}
...
\begin{ex}
Le taux de variation entre $-1$ et $3$ de la fonction $\fonction f {\R} {\R} x {x^2-x}$ est $\frac{f(3)-f(-1)}{3-(-1)}=\frac{6-2}{4}=1$.
\end{ex}
