...
path picture={
      \node[opacity=0.5,anchor=west,rotate=-20] at (path picture bounding box.north west) {%};}
        \includegraphics[scale=0.5]{Fondmoitie}};}
}
}
...
path picture={
      \node[opacity=0.03,rotate=10] at ($(path picture bounding box.west)$) {
        \includegraphics[scale=1.4]{FondNegatifmoitie}};}
]
(current page.south west) rectangle ($(current page.north east)+(3,3)$);
...
inner sep=0pt,
anchor=east,
] (maths) at ($(current page.east)+(1.5,0)$)
{
{\fontsize{35}{42} \selectfont \bf LGT Jean Rostand}
};
...
{
{\fontsize{30}{36}\selectfont\bf 1\textsuperscript{ère} ST2S}
};

\draw[ultra thick,gray] ($(annee.north west)+(0,0.2)$) -- ($(annee.north east)+(0,0.2)$);

% Title
\node[align=center,anchor=north east,inner sep=0pt] at ($(current page.east)+(-1.1,-6)$) 
{
{\fontsize{50}{60} \selectfont\scshape {Mathématiques}} \\[1cm]
{\fontsize{20}{24} \selectfont \textcolor{orange}{ \bf {M. DELAUNEY}}}
};
\end{tikzpicture}
