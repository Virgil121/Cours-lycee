...
Pour définir une suite arithmétique $g$, on a besoin de deux nombres:
\begin{itemize}
...
\rem[eleve]{On se restreindra au cas $q>0$.}
...
\addplot[samples=11,domain=(0:10),only marks,mark=square*,semithick,fill=red]{50*1.25^x};
\addplot[samples=11,domain=(0:10),only marks,mark=triangle*,semithick,fill=green]{-400*1.1^x};
