...
\fancyhead[C]{\textbf{DS4 : Fonctions affines - Sujet 2}}
...
{
\begin{exercice} \
Sur le repère ci-contre, tracer les droite $d_1$ et $d_2$ d'équation respective $y=-3x+1$ et $y=\frac 1 3 x -3$.
\end{exercice}

\begin{exercice} \
Déterminer algébriquement (par le calcul) l'équation réduite de la droite passant par les points $A(-3;-6)$ et $B(6;12)$.
...
\addplot[styleplot,domain=(-5:5)]{3*x-4} node[pos=0.7,right] {$d_3$};
\addplot[styleplot,domain=(-6:6)]{-4/5*x+2} node[pos=0.9,above right] {$d_4$};
\end{axis}
\end{tikzpicture}
\end{centrer}
}
