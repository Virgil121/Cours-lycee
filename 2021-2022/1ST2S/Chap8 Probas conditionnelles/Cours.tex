...
\newcommand{\card}[1]{\ensuremath{\textrm{Card}\left(#1\right)}}
% Début du document
%%%%%%%%%%%%%%%%%%%
\begin{document}
...
\section{Quelques rappels de vocabulaire ensembliste}

\begin{defins}
On se donne $E$ un ensemble et $A$,$B$ deux sous-ensembles de $E$.
\begin{itemize}
\item \textbf{L'intersection} de $A$ et $B$ notée $A\cap B$ désigne les éléments de $E$ appartenant à la fois à $A$ et à $B$.
\item \textbf{L'union} de $A$ et $B$ notée $A\cup B$ désigne  les éléments de $E$ appartenant à $A$ ou à $B$ ou aux deux.
\item \textbf{Le complémentaire} de $A$, noté $\overline A$ désigne les éléments de $E$ qui n'appartiennent pas à $A$.
\end{itemize}
\end{defins}
...
\begin{itemize}
    \item $A\cap B$ désigne les chiens au poil blanc et beige
    \item $A\cup B$ désigne les chiens au poil blanc, beige ou les deux à la fois.
    \item $\overline A$ désigne les chiens au poil autre que beige.
\end{itemize}
\end{ex}
...
\begin{ex}
Au sein d'une classe de 1ST2S de 35 élèves, il y a 23 filles. De plus, 7 garçons ont choisi l'allemand contre seulement 3 filles. On peut alors dresser le tableau suivant:
...
\begin{itemize}
    \item Le nombre d'éléments de $F \cap A$ (c'est-à-dire le nombre des filles ayant choisi l'allemand) est 3.
...
\rem[eleve]{De la même manière qu'avec les fréquences conditionnelles, il est possible de définir des probabilités conditionnelles.}

\begin{defin}
Soient $A$ et $B$ deux événements tels que $\Pro(A)\neq 0$. On note $\Pro_A(B)$ la probabilité de $B$ en sachant que $A$ est réalisé, aussi appelée probabilité de $B$ sachant $A$. On a de plus $\mathbb{P}_{A}(B)=\dfrac{\mathbb{P}(A\cap B)}{\mathbb{P}(A)}$.
\end{defin}

\rem[eleve]{On utilisera aussi des tableaux pour trouver les probabilités conditionnelles.}
...
\begin{center}\renewcommand{\arraystretch}{1.2}
    ...
    \cline{2-3} \multicolumn{1}{c|}{}
         &Activité sportive $(S)$ & Pas d'activité sportive ($\overline{S}$)  \\ \hline
...
Soient $A$ et $B$ deux événements tels que $\mathbb{P}(A)\neq 0.$ De la même manière qu'avec les fréquences conditionnelles il est possible de définir des probabilités conditionnelles.
