...
Développons cette expression: $\begin{aligned}[t]f(x)&=3\left( x^2-x \times x_2-x+x_2 \right)\\&= 3x^2+{\color{DarkRed}(3-3x_2)}x+{\color{DarkBlue}3x_2}\end{aligned}$

On a de plus $f(x)=3x^2 {\color{DarkRed}-9}x+{\color{DarkBlue}6}$.
...
\begin{prop}
Soit $f:x \mapsto a(x-x_1)(x-x_2)$ une fonction de degré 2. On suppose que $x_1 < x_2$. Le tableau de signes de $f$ dépend du signe de $a$:
...
\begin{tikzpicture}
\tkzTabInit[lgt=1.5,espcl=1.5]{$x$ /1, $f(x)$/1}{$- \infty$, $x_1$, $x_2$, $+ \infty$}
\tkzTabLine{,+,z,-,z,+,}
\end{tikzpicture}
\end{centrer}
}
